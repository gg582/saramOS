/*
 * Minimal 8x8 bitmap font for LVGL v9 (ASCII 32-126).
 * Public-domain style fallback glyphs.
 */
#include "lv_font.h"
#include <stdint.h>
...
static const uint8_t font_bitmap[GLYPH_COUNT * GLYPH_HEIGHT] = {
    /* 0x20 ' ' */
...
static uint8_t get_glyph_dsc(const lv_font_t *font, uint32_t letter,
                             int16_t *ofx, int16_t *ofy, uint8_t *w, uint8_t *h)
{
    (void)font;
    if (letter < FIRST_GLYPH || letter >= FIRST_GLYPH + GLYPH_COUNT) {
        *w = 0;
        *h = 0;
        *ofx = 0;
        *ofy = 0;
...
int16_t lv_font_get_glyph_width(const lv_font_t *font, uint32_t letter)
{
    int16_t ofx, ofy;
    uint8_t w, h;
    if (font && font->get_glyph_dsc && font->get_glyph_dsc(font, letter, &ofx, &ofy, &w, &h)) {
        return w;
    }
    return 0;
}
...
{
    if (font) return font->line_height;
    return 0;
}
