...
#define GPIOA_OTYPER    (*(volatile uint32_t *)(GPIOA_BASE + GPIO_OTYPER))
...
#define SDMMC_CMD_WAITRESP_SHORT_NOCRC (2U << 6)  /* for R3 responses */
...
#define USART_ISR_NE    (1U << 2)   /* Noise error */
#define USART_ICR_ORECF (1U << 3)   /* Clear overrun error flag */
#define USART_ICR_FECF  (1U << 1)   /* Clear framing error flag */
#define USART_ICR_NCF   (1U << 2)   /* Clear noise error flag */
#define USART_CR3_OVRDIS (1U << 12) /* Disable overrun detection */
